...
@property (nonatomic,strong)NSMutableArray *frPersonAry;
...
- (NSArray*)process:(ASF_CAMERA_DATA*)cameraData
{
    ...
    if(0 == dispatch_semaphore_wait(_processSemaphore, 0))
    {
        ...
        do {
            ...
            NSLog(@"------%lu------",(unsigned long)arrayFaceInfo.count);
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(){
                ...
                if(detectFace)
                {
                    ...
                    if(mr == ASF_MOK)
                    {
                        ...
                        float maxScore = 0.0;
                        [self.frPersonAry removeAllObjects];
                        for (ASFRPerson* person in persons)
                        {
                            ...
                            if (mr == ASF_MOK && fConfidenceLevel >= maxScore) {
                                ...
                                MFloat scoreThreshold0 = 0.61;
                                if (maxScore > scoreThreshold0) {
                                    [self.frPersonAry addObject:person.name];
                                }
                            }
                            
                        }
                        MFloat scoreThreshold = 0.61;
                ...
                dispatch_sync(dispatch_get_main_queue(), ^{
                    ...
                    if (self.delegate&&[self.delegate respondsToSelector:@selector(processRecognized:personAry:allmachAry:)])
                    {
                        [self.delegate processRecognized:self.frPerson.name personAry:arrayFaceInfo allmachAry:self.frPersonAry];
                        [self.frPersonAry removeAllObjects];
                    }
...
-(NSMutableArray *)frPersonAry {
    if (!_frPersonAry) {
        _frPersonAry = [NSMutableArray array];
    }
    return _frPersonAry;
}
@end
