...
-(void)fillWithModel:(PSConsultation*)model{
    self.nameLab.text=[PSSessionManager sharedInstance].session.families.name?[PSSessionManager sharedInstance].session.families.name:[LXFileManager readUserDataForKey:@"phoneNumber"];
    NSString*avatarUrl=PICURL([PSSessionManager sharedInstance].session.families.avatarUrl);
