...
- (void)setPrisons:(NSArray *)Prisons {
    ...
    for (int i = 0; i<_Prisons.count+1;i++ ) {
        ...
        UILabel *keyLabel = [[UILabel alloc] initWithFrame:CGRectMake(iconImage.right+5,40*i,150, 40)];
        NSString*Inmates=NSLocalizedString(@"Inmates:", @"服刑人员:");
        keyLabel.text = Inmates;
        ...
        if (i<_Prisons.count) {
            MeetPrisonserModel *meetJails = _Prisons[i];
            valueLab.text = meetJails.prisonerName;
        } else {
            valueLab.text = @"";
        }
    }
...
- (void)changAction:(UIButton *)sender {
    ...
    MeetPrisonserModel *meetjails = _Prisons[tag-10];
